...
items <- read.csv('survey_template.csv')
itemcodes <- seq(4,36,2) #X Codes in JISC file corresponding to items
...
nitems<-nrow(items)
for (i in 1:nitems){
  nopt <- items$N.options[i]
  topopt <- items$hirange[i]
  itemcode<-paste0('x',itemcodes[i],'_')
...
#x is variable denoting both group and item

p <- ggplot(plotdata) +      
  
  # Add the stacked bar
  geom_bar(aes(x=x, y=tot, fill=as.factor(response)), stat="identity", alpha=0.5) +
...
annotate("text",label="Group 1",x=3,y = 80)+
annotate("text",label="Group 2",x=7,y = 80)+
annotate("text",label="Group 3",x=11,y = 80)+
    # Add labels on top of each bar
 geom_text(data=label_data, aes(x=x, y=-5, label=item), color="black",size=3) +
  coord_polar()+
ylim(-20,100)
...
for (i in 1:nitems){
  ...
  datax <- df[,c(2,3,w)]
  names(datax)[(w-3)]<-LETTERS[1:nopt]
...
Here is code from https://bookdown.org/content/b298e479-b1ab-49fa-b83d-a57c2b034d49/part.html#circular-stacked-barchart
```{r examplecode}
# library
library(tidyverse)
library(viridis)
...
data <- data %>% gather(key = "observation", value="value", -c(1,2)) #final term specifies cols to leave ungathered
