...
```{r makeplots, echo=F}
makeplots <- function(myjisc,jisccodes){
njisc<-nrow(myjisc)
for (i in 1:njisc){
  nopt <- myjisc$N.options[i]
  itemcode<-paste0('x',jisccodes[i],'_')
...
maketab <- function(myjisc,itemnum,df){
  jisccodes <- seq(4,36,2) #X Codes in JISC file corresponding to items
    ...
    rownames(myt)<-myjisc[itemnum,(ww+1):(ww+myjisc$hirange[i])]
    knitr::kable(myt)
}
```
...
`r mytext <- df$x11_4b_opt; gatherfree(mytext)`  
...
- A.Lack of co-ordination between relevant research actors, such as institutions, funders and publishers		- B. Conflict of interest for institutions investigating their own researchers					
- C. Lack of resources					
- D. Lack of expertise					
...
`r mytext <- df$x13_5b_opt; gatherfree(mytext)` 	
...
`r mytext <- df$x15_6b_opt; gatherfree(mytext)`  
...
`r mytext <- df$x17_7b_opt; gatherfree(mytext)` 
...
`r mytext <- df$x19_8b_opt; gatherfree(mytext)` 
...
`r mytext <- df$x21_9b_opt; gatherfree(mytext)` 
