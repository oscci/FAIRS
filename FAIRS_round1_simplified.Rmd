...
thisf <- "cm46vj4fd001i1020aww1cwpy-analytics-uncoded-11-01-2025.csv"
...
makemeanstab <- function(myjisc,itemnum,df,thisrow){
  jisccodes <- seq(4,36,2) #X Codes in JISC file corresponding to items
    ...
    names(sumtab)[2:4]<-paste0(names(sumtab[2:4])," (N = ", Ntab$x,")")
...
makecattab <- function(myjisc,itemnum,df,thisrow,myterms){
  jisccodes <- seq(4,36,2) #X Codes in JISC file corresponding to items
    ...
    myrow<-nrow(sumtab)+1
    sumtab[myrow,]<-'-'
    sumtab[myrow,1]<-'Your response'
    mygroup <- as.numeric(df$group[thisrow])
    sumtab[myrow,(1+mygroup)]<-LETTERS[df[thisrow,w]]
    }
...
For items coded quantitatively, we show the mean and (SD) for each subgroup, where subgroup 1 = S(leuths), 2 = R(esearch Integrity Officers) and 3 = O(ther).  
...
## Free text comments
```{r reorderbygroup, echo=F}

#sort by group: makes it easier when looking at free text at the end
